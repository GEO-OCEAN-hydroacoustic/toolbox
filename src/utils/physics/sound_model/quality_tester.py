...
class LocalizationQualityTester:
    """
    Tests statistiques pour évaluation de localisation sismique
    Basé sur Note de cours ENSTA-Bretagne
    """
    ...
    def test_individual_residuals_student(self, residuals: np.ndarray) -> dict:
        """
        Cas H0 non rejetée : σ₀² = 1 vraisemblable mais estimé.
        On utilise σ̂²₀ estimé → résidus studentisés suivent Student(n-p).
        """
        n = len(residuals)
        dof = n - self.n_params
        ...
        # Résidus studentisés par σ̂₀ estimé
        tau_scores = np.abs(residuals) / sigma_hat if sigma_hat > 0 else np.abs(residuals)

        # Seuil Student(n-p)
        threshold = student_t.ppf(1 - self.alpha_outlier / 2, dof)
        ...
        return {
            ...
            'sigma_hat': sigma_hat,
            'w_scores': tau_scores  # clé conservée pour compute_selection_score
        }

    def test_individual_residuals_normal(self, residuals: np.ndarray) -> dict:
        """
        Cas H0 rejetée : matrice de cov supposée correcte, on impose σ̂² = 1.
        Les résidus (déjà normalisés) suivent N(0,1).
        """
        n = len(residuals)

        threshold = norm.ppf(1 - self.alpha_outlier / 2)  # ≈ 3.29

        # σ imposé = 1, pas de re-normalisation
    ...
    def compute_selection_score(self, global_result: dict,
        ...
        # Bonus observations
        score_obs = np.log(n_obs) / np.log(12)  # Normalisé ~[0.5, 1] pour n_obs ∈ [3, 10]
    ...
    def evaluate(self, result) -> QualityReport:
        ...
        if global_result['passes']:
            # H0 non rejetée : σ̂²₀ estimé → Student(n-p)
            outlier_result = self.test_individual_residuals_student(residuals)
        else:
            # H0 rejetée : σ imposé = 1 → N(0,1)
            outlier_result = self.test_individual_residuals_normal(residuals)
    ...
    def _determine_quality_level(self, global_result: dict,
        ...
        if passes_global and n_outliers <= 1 and global_result['dof'] >=6 :
            return QualityLevel.GOOD

        # Marginal : test global échoue mais σ² pas trop loin de 1
        if 0.5 < sigma2 < 2.0 and n_outliers <= 1:
