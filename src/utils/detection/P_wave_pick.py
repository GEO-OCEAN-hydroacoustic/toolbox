...
#         self.filtered_signal_line1 = None  # For first filter
#         self.filtered_signal_line2 = None  # For second filter
#         self.theoretical_lines = []
#         self.raw_signal = None  # Store raw signal
#         self.sampling_frequency = 240  # Default sampling frequency
#         self.current_filter = 1  # Default filter
#
...
#         self.filter_group = QGroupBox("Filter Options")
#         self.filter_layout = QVBoxLayout()
#
#         # Filter selection radio buttons
#         self.filter_radio_group = QButtonGroup(self)
#         self.filter1_radio = QRadioButton("Filter 1 (0.6-2.1 Hz)")
#         self.filter2_radio = QRadioButton("Filter 2 (0.06-2.1 Hz)")
#         self.raw_radio = QRadioButton("Raw Signal")
#         self.filter1_radio.setChecked(True)  # Default to filter 1
#
#         self.filter_radio_group.addButton(self.filter1_radio, 1)
#         self.filter_radio_group.addButton(self.filter2_radio, 2)
#         self.filter_radio_group.addButton(self.raw_radio, 0)
#
#         self.filter_layout.addWidget(self.filter1_radio)
#         self.filter_layout.addWidget(self.filter2_radio)
#         self.filter_layout.addWidget(self.raw_radio)
#
...
#         """Setup the bandpass filter coefficients"""
#         # Sampling frequency
#         self.Fs = 240  # Hz
#
#         # Filter 1 (0.6-2.1 Hz bandpass) - "Red curve"
#         self.b1, self.a1 = signal.butter(2, [0.6 / (self.Fs/2 ), 2.1 / (self.Fs/2 )], 'bandpass')
#
#         # Filter 2 (0.06-2.1 Hz bandpass) - "Black curve"
#         self.b2, self.a2 = signal.butter(2, [0.06 / (self.Fs/2 ), 2.1 / (self.Fs/2 )], 'bandpass')
#
#     def apply_filter(self, data, filter_type=1):
#         """Apply the selected bandpass filter"""
#         if filter_type == 1:
#             # Apply Filter 1 (0.6-2.1 Hz)
#             filtered = signal.filtfilt(self.b1, self.a1, data,  padtype='odd', padlen=3*(max(len(self.b1),len(self.a1))-1))
#         elif filter_type == 2:
#             # Apply Filter 2 (0.06-2.1 Hz)
#             filtered = signal.filtfilt(self.b2, self.a2, data,  padtype='odd', padlen=3*(max(len(self.b2),len(self.a2))-1))
#         else:
#             # No filter (raw signal)
#             return data
#
#         return filtered
#
...
#         self.filter_radio_group.buttonClicked.connect(self.change_filter)
#
#     def change_filter(self, button):
#         """Change the current filter and update plot"""
#         self.current_filter = self.filter_radio_group.id(button)
#
#         # If we have signal data, update the plot
#         if self.raw_signal is not None:
#             self.update_plot_with_current_filter()
#
#     def update_plot_with_current_filter(self):
#         """Update the plot with the current filter settings"""
#         if self.raw_signal is None:
#             return
#
#         # Get time array
#         time = np.arange(len(self.raw_signal)) / self.sampling_frequency
#
#         # Clear existing signal lines
#         if self.signal_line:
#             self.signal_line.remove()
#             self.signal_line = None
#
#         # Plot appropriate signal based on selected filter
#         if self.current_filter == 0:
#             # Raw signal
#             self.signal_line, = self.ax.plot(time, self.raw_signal, 'k', label='Raw Signal', alpha=0.7)
#         elif self.current_filter == 1:
#             # Filter 1 (0.6-2.1 Hz)
#             filtered = self.apply_filter(self.raw_signal, 1)
#             self.signal_line, = self.ax.plot(time, filtered, 'r', label='Filter 0.6-2.1 Hz', alpha=0.7)
#         elif self.current_filter == 2:
#             # Filter 2 (0.06-2.1 Hz)
#             filtered = self.apply_filter(self.raw_signal, 2)
#             self.signal_line, = self.ax.plot(time, filtered, 'b', label='Filter 0.06-2.1 Hz', alpha=0.7)
#
#         # Update legend
#         handles, labels = self.ax.get_legend_handles_labels()
#
#         # Keep only theoretical line in legend if it exists
#         theoretical_handle = None
#         for handle, label in zip(handles, labels):
#             if 'Theoretical' in label:
#                 theoretical_handle = (handle, label)
#                 break
#
#         if theoretical_handle:
#             self.ax.legend([self.signal_line, theoretical_handle[0]],
#                           [self.signal_line.get_label(), theoretical_handle[1]])
#         else:
#             self.ax.legend([self.signal_line], [self.signal_line.get_label()])
#
...
#         # Redraw
#         self.canvas.draw()
#         filter_name = {0: "Raw", 1: "0.6-2.1 Hz", 2: "0.06-2.1 Hz"}
#         self.status_label.setText(f"Showing {filter_name[self.current_filter]} signal")
#
...
#
#         # Restore filter preference if available
#         if self.settings.contains("current_filter"):
#             filter_id = int(self.settings.value("current_filter"))
#             self.current_filter = filter_id
#             button = self.filter_radio_group.button(filter_id)
#             if button:
...
#         self.settings.setValue("current_filter", self.current_filter)
...
#
#             # Update sampling frequency from file
#             if hasattr(df, 'header') and 'sampling_frequency' in df.header:
#                 self.sampling_frequency = df.header['sampling_frequency']
#             else:
#                 self.sampling_frequency = 240  # Default if not found
#
#             # Update filter coefficients with current sampling frequency
#             self.setup_filters()
#
#             # Store raw signal data
#             self.raw_signal = df.data
#
#             # Create time array for the seismogram
#             time = np.arange(len(df.data)) / self.sampling_frequency
#
#             self.ax.clear()
#
#             # Apply and plot the currently selected filter
#             if self.current_filter == 0:
#                 # Raw signal
#                 self.signal_line, = self.ax.plot(time, self.raw_signal, 'k', label='Raw Signal', alpha=0.7)
#             elif self.current_filter == 1:
...
#                 line = self.ax.axvline(t, color='g', linestyle='--', alpha=1,
...
#             self.canvas.draw()
#             filter_name = {0: "Raw", 1: "0.6-2.1 Hz", 2: "0.06-2.1 Hz"}
#             self.status_label.setText(f"Ready to pick phases - Showing {filter_name[self.current_filter]} signal")
#
#         except Exception as e:
