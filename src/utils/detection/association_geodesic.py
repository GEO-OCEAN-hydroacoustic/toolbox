...
def squarize(coordinates, weights, lat_bounds, lon_bounds, size=1000):
    ...
    lat_res, lon_res = (lat_bounds[-1] - lat_bounds[0]) / size, (lon_bounds[-1] - lon_bounds[0]) / size

    lats = np.sort(np.unique(coordinates[:,0]))
    lat_size = round(((lats[1] - lats[0])) / lat_res)
    for ilat, lat in enumerate(lats):
        idx = coordinates[:,0] == lat
        lons = coordinates[idx,1]
        weights_parallel = weights[idx]

        lon_size = round(((lons[1] - lons[0])) / lon_res)
        for ilon in range(len(lons)):
            grid[ilat * lat_size:(ilat + 1) * lat_size, ilon * lon_size:(ilon + 1) * lon_size] = weights_parallel[ilon]
...
def _compute_parallel(lat, lat_step, lon_bounds, step_m, stations, corner_coordinates,
    ...
    lon_m = distance_point_point((lat, lon_bounds[0]),
                                 (lat, lon_bounds[1]), fast=False)
    m_per_deg = lon_m / (lon_bounds[1] - lon_bounds[0])
    lon_step = step_m / m_per_deg
    first_lon = lon_bounds[0] + lon_step / 2
    last_lon = lon_bounds[1] - lon_step / 2
    n_steps = int(np.ceil(lon_m / step_m))
    lons = np.linspace(first_lon, last_lon, n_steps)
    for lon in lons:
...
def compute_grids(lat_bounds, lon_bounds, grid_size, sound_model, stations,
    ...
    # for this, we use latitudes (that should be constant)
    step_m = distance_point_point((lat_bounds[0], lon_bounds[0]),
                                        (lat_bounds[1], lon_bounds[0]), fast=False) / grid_size
    lat_step = ((lat_bounds[1] - lat_bounds[0]) / grid_size)
    first_lat = lat_bounds[0] + lat_step / 2
    last_lat = lat_bounds[1] - lat_step / 2
    lats = np.linspace(first_lat, last_lat, grid_size)
    ...
    with Pool(processes=max(1,cpu_count()//2)) as pool:
        results = pool.starmap(_compute_parallel, [(lat, lat_step, lon_bounds, step_m, stations, corner_coordinates,
                sound_model, pick_uncertainty, sound_speed_uncertainty, max_clock_drift) for lat in lats])

    for result in results:
        grid_to_coords.extend(result[0])
        for s in stations:
            for m in range(12):
    ...
    return grid_to_coords, TDoA, max_TDoA, TDoA_uncertainties